---
name: issue-loader
description: Fetches a GitHub issue by number and extracts structured metadata — title, labels, status, spec comment, branch slug, and body. Used by task and feature workflow skills to load issue context at the start of each step.
tools: Bash
model: haiku
---
...
- **branch** — derived as `task/{number}-{slug}` for task issues, `feature/{slug}` for feature issues
